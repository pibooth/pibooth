...
from pibooth.camera.hybrid import HybridRpiCamera, HybridRpi2Camera, HybridCvCamera


def close_proxy(rpi2_cam_proxy, rpi_cam_proxy, gp_cam_proxy, cv_cam_proxy):
    """Close proxy drivers.
    """
    if rpi2_cam_proxy:
...
def find_camera():
    ...
    if rpi2_cam_proxy and gp_cam_proxy:
        ...
        close_proxy(None, rpi_cam_proxy, None, cv_cam_proxy)
        return HybridRpi2Camera(rpi2_cam_proxy, gp_cam_proxy)
    elif rpi_cam_proxy and gp_cam_proxy:
        LOGGER.info("Configuring hybrid camera (Picamera + gPhoto2) ...")
        close_proxy(rpi2_cam_proxy, None, None, cv_cam_proxy)
        ...
        close_proxy(rpi2_cam_proxy, rpi_cam_proxy, None, None)
        return HybridCvCamera(cv_cam_proxy, gp_cam_proxy)
    elif gp_cam_proxy:
        LOGGER.info("Configuring gPhoto2 camera ...")
        close_proxy(rpi2_cam_proxy, rpi_cam_proxy, None, cv_cam_proxy)
        ...
        close_proxy(rpi2_cam_proxy, None, gp_cam_proxy, cv_cam_proxy)
        return RpiCamera(rpi_cam_proxy)
    elif cv_cam_proxy:
        LOGGER.info("Configuring OpenCV camera ...")
        close_proxy(rpi2_cam_proxy, rpi_cam_proxy, gp_cam_proxy, None)
