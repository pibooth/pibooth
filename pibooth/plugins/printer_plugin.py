...
class PrinterPlugin(object):
    ...
    def state_wait_do(self, cfg, app, events):
        if evtfilters.find_event(events, evtfilters.EVT_PIBOOTH_BTN_PRINT) and app.previous_picture_file and app.printer.is_installed():
    ...
    def state_print_do(self, cfg, app, events):
        if evtfilters.find_event(events, evtfilters.EVT_PIBOOTH_BTN_PRINT) and app.previous_picture_file:
            self.print_picture(cfg, app)
