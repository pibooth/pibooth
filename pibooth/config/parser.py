...
DEFAULT = odict((
    ("GENERAL",
        odict((
            ...
            ("password_enabled",
                (False,
                 "Enable password protection on settings menu",
                 "Password protection", ['True', 'False'])),
            ("password",
                ("0000",
                 "Password that will be asked to access the settings menu",
                 "Password", "")),
        ))
     ),
    ("WINDOW",
        odict((
            ...
            ("get_ready_delay",
                (1,
                 "Duration of the \"Get ready\" message before preview (0 if never shown)",
                 "Get ready message display time", [str(i) for i in range(0, 10, 1)])),
            ("processing_min_delay",
                (1,
                 "On 'processing' state: min delay to display the loading view (0 to display as fast as possible)",
                 "Processing mininum display time", [str(i) for i in range(0, 10, 1)])),
            ...
            ("finish_thanks_delay",
                (2,
                 "On 'finish' state: how long is displayed the thanks page (0 if never shown)",
                 "Finish thanks display time", [str(i) for i in range(0, 10, 1)])),
            ("wait_picture_delay",
                (-1,
    ...
    ("CAMERA",
        odict((
            ...
            ("autofocus",
                (False,
                 "Use autofocus when capturing a new image",
                 None, None)),
        ))
     ),
    ("PRINTER",
