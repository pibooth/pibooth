...
def write_the_shelve():
    ...
    global bandg_check
    try:
        shelve_registry['post'] = {'blog': blog_check.get(), 'email': email_check.get(), 'printer': print_check.get(),
                                   'event': g_event.get(), 'company': g_company.get(), 'blog_email': g_blog_email.get(),
                                   'numcopies': num_print_copies.get(), 'admin': admin_check.get(),
                                   'sound': sound_check.get(), 'bandg': bandg_check.get()}
    finally:
...
def SetUpEvent():
    ...
    for child in EventBox.winfo_children():
